// Adapted from http://www.webreference.com/programming/javascript/mk/column3/
(function colorPickerNamespace(){
	var cp = null;

	var imgBase = '/images/color-picker/';
	...
	function createColorPicker(container){
		...
		container.appendChild(cp.ColorPicker);
    ...
    {
      var brightness = ((cp.cpColor.r * 299 + cp.cpColor.g * 587 + cp.cpColor.b * 114) / 1000 / 255);
      cp.cpColor = '#'+hex(cp.cpColor.r)+hex(cp.cpColor.g)+hex(cp.cpColor.b);
    }
    else
    {
      var parsedColor = parseColor(cp.cpColor); 
    ...
    cp.cpInput.style.backgroundColor = cp.cpColor;
		cp.cpInput.style.color = brightness < 0.5 ? '#FFFFFF' : '#000000';
		hideColorPicker();
	}

	function attachColorPicker(input, container, noLg){
    ...
    cp.hColorDiv.style.backgroundColor = cp.cpInput.style.backgroundColor;
		...
		cp.ColorPicker.style.left = (inpPos.x + parseInt(cp.cpInput.offsetWidth) + 3) + "px";
		cp.ColorPicker.style.top  = (inpPos.y - 1) + "px"; //+parseInt(cp.cpInput.offsetHeight);
    ...
    else
    {
      ...
      {
        return {
          r: eval('0x' + text.charAt(0).times(2)),
          g: eval('0x' + text.charAt(1).times(2)),
          b: eval('0x' + text.charAt(2).times(2))
        }
      }
      else if (/^[0-9A-F]{6}$/i.test(text))
      {
			  return {
				  r: eval('0x' + text.substr(0, 2)),
				  g: eval('0x' + text.substr(2, 2)),
				  b: eval('0x' + text.substr(4, 2))
			  }
      }
		}
